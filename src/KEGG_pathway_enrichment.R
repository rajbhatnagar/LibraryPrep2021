...
## https://github.com/brunetlab/Leeman_et_al_2017/blob/master/kallisto_deseq2/KEGG_pathway_enrichment.R
## This code corresponds to the paper: Dena S. Leeman, Katja Hebestreit, et al, "Lysosome activation clears aggregates and enhances quiescent neural stem cell activation during aging"
##
##
...
geneset_enrichment_based_on_statistic <- function(gene.sets.file = "data/c2.cp.kegg.v7.1.entrez.gmt", 
    ...
    test.statistics <- lapply(gene.sets.l.red,
                              function(x){
                                  ...
                                  ## Number of genes going down and number of genes going up:
                                  num.genes.dn = sum(gene.statistics[, statistic] < 0)
                                  num.genes.up = sum(gene.statistics[, statistic] > 0)
                                  return(list(statistic = m,
                                              fold.change = avg.fold.change,
                                              num.genes.dn = num.genes.dn,
                                              num.genes.up = num.genes.up,
    ...
    results.df <- data.frame(p.val = p.values,
                             ...
                             num.genes = gene.set.lengths,
                             num.genes.dn = sapply(test.statistics, function(x) x$num.genes.dn),
                             num.genes.up = sapply(test.statistics, function(x) x$num.genes.up)
                             )
    results.df$percentage.dn = results.df$num.genes.dn / results.df$num.genes
    results.df$percentage.up = results.df$num.genes.up / results.df$num.genes
    ...
    ind.ord      <- order(results.df$adj.p.val)
